...
int main(int argc, char **argv)
{
    ...
    std::vector<float> In_rows(L1*DATA_SIZE);
    //std::vector<blockvec> C_rows(DATA_SIZE);
    std::vector<float> C_rows(L3*DATA_SIZE);
...
//    float C_ini[BSIZE*L4];
 //   float C_ref[BSIZE*L4];
  ...
  printf("flags set\n");
    ...
    cl::Buffer in1_buf(context, CL_MEM_USE_HOST_PTR|CL_MEM_READ_ONLY|CL_MEM_EXT_PTR_XILINX, sizeof(float) * L1*DATA_SIZE, &InrExt, &err);
    ...
    cl::Buffer out_buf(context, CL_MEM_USE_HOST_PTR|CL_MEM_WRITE_ONLY|CL_MEM_EXT_PTR_XILINX, sizeof(float) * L3*DATA_SIZE, &CrExt, &err);
      printf("hi\n");
